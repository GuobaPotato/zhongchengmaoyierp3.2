...
import { InventoryStockInDetail } from './InventoryStockInDetail';
...
const MOCK_DATA: StockInItem[] = [
  ...
  },
  {
    ...
    source: "TR-99283",
    operator: "王五",
    time: "2024-10-22 09:15",
    remark: "车间余料回库"
  }
];

export const InventoryStockIn: React.FC = () => {
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [searchTerm, setSearchTerm] = useState("");
  const [dateRange, setDateRange] = useState({ start: "2024-10-01", end: "2024-10-07" });
  const [viewingId, setViewingId] = useState<string | null>(null);

  if (viewingId) {
    return <InventoryStockInDetail stockInId={viewingId} onClose={() => setViewingId(null)} />;
  }
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">
          <div className="space-y-1.5">
            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">搜索入库单编号</label>
        ...
        <div className="flex items-center space-x-3">
          <div className="relative group">
            ...
            <div className="absolute top-full left-0 mt-2 w-48 bg-white border border-slate-200 rounded-2xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-40 p-2">
              <button className="w-full text-left px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 rounded-xl">批量确认入库</button>
              <button className="w-full text-left px-4 py-2 text-xs font-bold text-slate-600 hover:bg-slate-50 rounded-xl">批量打印标签</button>
              <button className="w-full text-left px-4 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50 rounded-xl border-t border-slate-50 mt-1">批量撤销单据</button>
            </div>
          </div>
        </div>
        ...
        <table className="min-w-full text-left border-collapse">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] whitespace-nowrap">
              ...
              <th className="px-4 py-5">入库单号</th>
              <th className="px-4 py-5">库位</th>
              <th className="px-4 py-5">入库类型</th>
              <th className="px-4 py-5">来源单号</th>
              ...
              <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                ...
                <td className="px-4 py-5">
                   <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                     item.type === '采购入库' ? 'bg-indigo-100 text-indigo-700' : 
                     item.type === '生产入库' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'
                   }`}>
                     {item.type}
                   </span>
                </td>
      ...
      <div className="bg-white px-10 py-6 rounded-[2rem] border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
        <div className="flex items-center space-x-12">
          <div className="text-sm font-bold text-slate-400 uppercase tracking-widest">
            Total Results: <span className="font-black text-slate-800 text-lg">3</span>
          </div>
        </div>
