...
const MOCK_POOL_DATA: LeadPoolItem[] = [
  {
    id: "P1",
    name: "广东恒洁水龙头制造有限公司",
    contact: "楼经理",
...
export const CRMLeadPool: React.FC = () => {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === MOCK_POOL_DATA.length) {
      ...
      setSelectedIds(new Set(MOCK_POOL_DATA.map(l => l.id)));
    }
  };
    ...
    <div className="flex flex-col h-full space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3 shrink-0">
          <button className="px-5 py-3 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center space-x-2 shadow-xl shadow-indigo-100">
      ...
      <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
          ...
          <table className="min-w-[2400px] text-left border-collapse">
            <thead className="bg-slate-50 border-b border-slate-100">
              <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] whitespace-nowrap">
                ...
                <th className="px-6 py-5 sticky left-12 bg-slate-50 z-20 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] border-r border-slate-100 min-w-[280px]">线索名称</th>
            ...
            <tbody className="divide-y divide-slate-50">
              {MOCK_POOL_DATA.map((lead) => (
                <tr key={lead.id} className="hover:bg-slate-50/50 transition-colors group">
                  ...
                  <td className="px-6 py-5 sticky left-12 bg-white group-hover:bg-slate-50 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.05)] border-r border-slate-100">
                    <span 
                  ...
                  <td className="px-4 py-5">
                    <span className="px-3 py-1 bg-amber-50 text-amber-700 rounded-lg text-[10px] font-black border border-amber-100 uppercase tracking-tighter">{lead.intendedProduct}</span>
                  </td>
                  ...
                  <td className="px-4 py-5">
                    <span className="text-[11px] font-black text-slate-300 italic border border-dashed border-slate-200 px-2 py-0.5 rounded-lg">{lead.owner}</span>
                  </td>
                  <td className="px-4 py-5 text-[11px] font-black text-slate-300 italic">{lead.ownerDept}</td>
                  ...
                  <td className="px-4 py-5 text-[12px] font-mono text-slate-400">{lead.nextFollowUp}</td>
        ...
        <div className="px-10 py-6 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 bg-slate-50/50 shrink-0">
          <div className="flex items-center space-x-12">
            <div className="flex items-center space-x-4">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">公海池操作:</span>
            ...
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              总计 <span className="text-slate-800 text-sm">8</span> 项资产 | 20条/页
            </div>
          </div>
